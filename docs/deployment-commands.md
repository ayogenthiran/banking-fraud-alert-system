...
## 3. First Terraform Apply Using Placeholder Image

This creates the AWS infrastructure, including the ECR repository, before the project image exists.

```bash
terraform apply \
  -var="project_name=banking-fraud-alert-system" \
  -var="aws_region=us-east-1" \
  -var="api_image_uri=public.ecr.aws/docker/library/python:3.11-slim"
```
...
docker tag banking-fraud-alert-system:latest $ECR_REPO_URL:latest
...
terraform apply \
  -var="project_name=banking-fraud-alert-system" \
  -var="aws_region=us-east-1" \
  -var="api_image_uri=$ECR_REPO_URL:latest"
```
